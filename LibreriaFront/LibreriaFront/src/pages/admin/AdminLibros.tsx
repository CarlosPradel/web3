    ...
    <div className="min-h-screen w-screen bg-gradient-to-br from-blue-100 via-white to-blue-100 px-6 py-10">
      <div className="max-w-6xl mx-auto bg-white shadow-2xl rounded-3xl p-8">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-3xl font-extrabold text-blue-700">Gestión de Libros</h2>
          <button
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {libros.map((libro) => (
            <li key={libro.id} className="list-none border bg-white p-6 rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-200">
              <div className="flex gap-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="space-y-1 text-gray-600">
                    ...
                    <p><span className="font-medium">Precio:</span> Bs{libro.precio}</p>
                    <p><span className="font-medium">ISBN:</span> {libro.isbn}</p>
                    <p className="flex flex-wrap gap-1">
                      <span className="font-medium">Géneros:</span>
                      {libro.generos.map((g) => (
                        <span key={g.id} className="inline-block bg-blue-100 text-blue-800 text-sm px-2 py-1 rounded">
                          {g.nombre}
                        </span>
                      ))}
                    </p>
